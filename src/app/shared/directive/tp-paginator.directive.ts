...
@Directive({
  selector: '[appTpPaginator]',
  standalone: true,
})
export class TpPaginatorDirective implements AfterViewInit {
  /**
   * custom emitter for parent component
   */
  @Output()
  pageIndexChangeEmitter: EventEmitter<number> = new EventEmitter<number>();
  ...
  private styleDefaultPagination() {
    ...
    this.ren.addClass(nextButtonDefault, 'g-bubble');
    this.ren.setStyle(nextButtonDefault, 'margin-inline-start', '8px');
    this.ren.addClass(previousButton, 'g-bubble');
  ...
  private createButton(i: number): HTMLElement {
    ...
    this.ren.setStyle(bubbleButton, 'margin-inline-start', '8px');
  ...
  private createDotsElement(): HTMLElement {
    ...
    // add class
    this.ren.addClass(dotsEl, 'g-dot-el');
